...
export class SignInAdminComponent {
  //usuarios
  public usuario: Administrador; // Cambio de Usuario a Administrador
  ...
  constructor(
    ...
    //this.usuario = new Administrador(0, '', 'Name', 'Lastname', 'ejemplo@olx8.com', '0991125207', 'hombre',
    // '', '2020-01-01', 'Quito,Distrito', '1234567890', 1, 1, 1);
    this.usuario = new Administrador(0, '', '', '', '', '', '', '', '', '', '', 0, 0, 0); // Cambio de Usuario a Administrador
    this.foto = '';
  ...
  guardarUsuario(form: NgForm) {
    ...
    this.usuario.contrasenia = this.contrasenia; // Cambio de Usuario a Administrador
    this._usuarioService.guardarAdministrador(this.usuario).subscribe({ // Cambio de Usuario a Administrador
      next: (response) => {
        if (this.token == undefined) {
          localStorage.setItem('token', response.token);
          localStorage.setItem('id_usuario', response.id_administrador.toString());
        }        if (this.archivoSeleccionado) {
          this._usuarioService.updateImage(response.id_administrador, this.archivoSeleccionado as File).subscribe({
            error: (e: HttpErrorResponse) => {
              this._errorService.msjError(e);
            }
          });
        ...
        this.toastr.success(`${this.usuario.nombre} ${this.usuario.apellido} haz sido registrado con éxito`, 'BIENVENIDO'); // Cambio de Usuario a Administrador
      },
      error: (e: HttpErrorResponse) => {
        this._errorService.msjError(e);
  ...
  calcularUbicacion(latitud: number, longitud: number) {
    this.http.get(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitud}&lon=${longitud}&zoom=10&addressdetails=1`)
      .subscribe((response: any) => {
        this.usuario.ubicacion = response.display_name; // Cambio de Usuario a Administrador
      });
  }
